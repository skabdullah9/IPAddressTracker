<template>
  <div class="min-h-screen h-screen max-h-screen flex flex-col">
    <header class="flex flex-col pt-2 md:pt-4 pb-12 md:pb-24 px-2 relative">
      <h1 class="font-mono text-3xl mb-2 md:mb-5 text-gray-200">
        IP Address Tracker
      </h1>
      <form
        @submit.prevent="getIpInfo"
        class="search flex max-w-xl w-full mx-auto font-mono "
      >
        <input
          v-model="queryIp"
          placeholder="Search any IP address or leave it blank to fetch current IP"
          type="text"
          class="w-full text-base md:text-2xl py-1 px-3 focus:outline-none rounded-tl-md rounded-bl-md"
        />
        <button
          type="submit"
          @click="getIpInfo"
          class="bg-black p-2 h-full rounded-tr-md rounded-br-md"
        >
          <svg
    ...
    const getIpInfo = async () => {
      try {
        const response = await axios(
          `https://geo.ipify.org/api/v1?apiKey=at_BtlDH0GZhLrj3qwiPgRyGlOyS0898&ipAddress=${queryIp.value}`
        );
